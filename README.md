...
## What does it do?
Redux-phunk allows you to dispatch an array of actions that will be executed in sequence, instead of just one action. For asynchronous code such as ajax, a special type of object (called a `phunk`) is included in the array, which all subsequent actions in the sequence will wait for.
